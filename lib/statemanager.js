...
StateManager.prototype.queueRawTransaction = function(rawTx, callback) {
  ...
  this.queueTransaction("vap_sendRawTransaction", txParams, callback);
};
...
StateManager.prototype.queueTransaction = function(method, tx_params, callback) {
  ...
  if (method == "vap_sendTransaction" && this.unlocked_accounts[tx_params.from] == null) {
...
StateManager.prototype.processNextAction = function(override) {
  ...
  if (queued.method == "vap_getStorageAt") {
  ...
  } else if (queued.method == "vap_sendTransaction" || queued.method == "vap_sendRawTransaction") {
...
StateManager.prototype.sign = function(address, dataToSign) {
  ...
  var sgn = utils.ecsign(Buffer.from(dataToSign.replace('0x',''), 'hex'), Buffer.from(secretKey));
